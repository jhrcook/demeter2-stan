...
model {
    ...
    sigma_gbar ~ normal(0, 3.0);
    sigma_g ~ normal(0, 3.0);
    ...
    
    {
        vector[N] y_hat;
        for (n in 1:N) {
            y_hat[n] = a[cell_line[n]] + c[shrna[n]] + alpha[shrna[n]] * (gbar[gene[n]] + g[cell_line[n], gene[n]]);
...
generated quantities {
    ...
    for (n in 1:N)
        y_pred[n] = normal_rng(a[cell_line[n]] + c[shrna[n]] + alpha[shrna[n]] * (gbar[gene[n]] + g[cell_line[n], gene[n]]), sigma[shrna[n]]);
}
