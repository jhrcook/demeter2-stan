...
model {
    ...
    
    {
        vector[N] y_hat;
        for (n in 1:N) {
            y_hat[n] = c[shrna[n]] + alpha[shrna[n]] * (gbar[gene[n]] + g[cell_line[n], gene[n]]);
...
generated quantities {
    ...
    for (n in 1:N)
        y_pred[n] = normal_rng(c[shrna[n]] + alpha[shrna[n]] * (gbar[gene[n]] + g[cell_line[n], gene[n]]), sigma[shrna[n]]);
}
